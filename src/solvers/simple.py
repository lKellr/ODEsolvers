...
def Euler(
    ...
    h: float,
    t0: float = 0.0,
) -> tuple[NDArray[np.floating], NDArray[np.floating], dict[str, int]]:
...
def AB3(f, x0, t_max, h, t0=0):
    ...
    for i in range(2, steps - 1):
        # t[:,i+1:i+2]=t[:, i:i+1]+h
        x[:, i + 1 : i + 2] = x[:, i : i + 1] + h / 12 * (
            23 * f(t[:, i : i + 1], x[:, i : i + 1])
            - 16 * f(t[:, i - 1 : i], x[:, i - 1 : i])
            + 5 * f(t[:, i - 2 : i - 1], x[:, i - 2 : i - 1])
        )
...
def PECE(f, x0, t_max, h, nrep=1, t0=0):
    ...
    for i in range(2, steps - 1):
        ...
        k = x[:, i : i + 1] + h / 24 * (
            19 * f(t[:, i : i + 1], x[:, i : i + 1])
            - 5 * f(t[:, i - 1 : i], x[:, i - 1 : i])
            + f(t[:, i - 2 : i - 1], x[:, i - 2 : i - 1])
        )
        for j in range(nrep):
            x[:, i + 1 : i + 2] = k + 9 * h / 24 * f(
                t[:, i + 1 : i + 2], x[:, i + 1 : i + 2]
...
def DP45(f, t_max, x0, t0=0, eps=10**-5, s_clip=[0.2, 5], tol_safety=0.9):
    ...
    while t[k] < t_max:  # iterate until t_max is reached
        ...
        if (
            err < eps
        ):  # or h==h_min): #accept result if tolerance is met, or we cant decrease h anymore
            k1 = k2
            k += 1
            x.append(x_pred)
            t.append(t_pred)
...
def BDF2(f, x0, t_max, h, t0=0, solver="BFGS", solvertol=1e-5):
    ...
    for i in range(steps - 1):
        ...
        f_imp = (
            ...
            - 4 / 3 * x[:, i : i + 1]
            + 1 / 3 * x[:, i - 1 : i]
            - 2 / 3 * h * f(t[:, i + 1 : i + 2], x_next)
        )
        x[:, i + 1 : i + 2] = solver(
            f_imp, x[:, i : i + 1], np.eye(x0.shape[1]), solvertol
        )
    return t, x
